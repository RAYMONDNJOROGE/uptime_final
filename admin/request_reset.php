...
if (isset($_SESSION['otp_feedback'])) {
    $message = $_SESSION['otp_feedback']['message'] ?? '';
    $messageType = $_SESSION['otp_feedback']['type'] ?? '';
    $email = $_SESSION['otp_feedback']['email'] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        if (!$admin) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM otp_attempts WHERE ip_address = :ip AND email = :email");
            ...
            if ($canProceed) {
                // Insert or update attempt record AFTER checks
                if ($record) {
                    $pdo->prepare("UPDATE otp_attempts SET attempts = attempts + 1, last_attempt = NOW() WHERE email = ? AND ip_address = ?")
                        ->execute([$email, $ip]);
                } else {
                    $pdo->prepare("INSERT INTO otp_attempts (ip_address, attempts, email, last_attempt, first_attempt)
                                   VALUES (?, 1, ?, NOW(), NOW())")
                        ->execute([$ip, $email]);
                }
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .alert-success  { background: rgba(56, 161, 105, 0.15); color: #68d391; border: 1px solid rgba(56, 161, 105, 0.3); }
    .alert-info     { background: rgba(49, 130, 206, 0.15); color: #63b3ed; border: 1px solid rgba(49, 130, 206, 0.3); }
    .alert-warning  { background: rgba(221, 107, 32, 0.15); color: #f6ad55; border: 1px solid rgba(221, 107, 32, 0.3); }

    .form-group { margin-bottom: 20px; }
    .form-group label {
      display: block; margin-bottom: 8px;
      font-weight: 600; font-size: 14px; color: var(--gray-light);
    }
    ...
    .divider { height: 1px; background: var(--gray); margin: 25px 0; }
  ...
  <div class="container">
    ...
    <?php if ($message): ?>
      <div class="alert alert-<?= $messageType ?>">
        <i class="fas fa-<?= $messageType === 'error' ? 'exclamation-circle' : ($messageType === 'success' ? 'check-circle' : ($messageType === 'warning' ? 'exclamation-triangle' : 'info-circle')) ?>" aria-hidden="true"></i>
